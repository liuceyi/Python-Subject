...
hourly_data = pd.read_csv('Data/Hourly_ReGen.csv', header=0, index_col=0)
...
monthly_data = pd.read_csv('Data/MonthlyFloating_ReGen.csv', header=0, index_col=0)
...
# The root node is indexed by '0' and the next node will be indexed by len(tree)-1-1
...
# Define a function to perform MCTS algorithm k times from a given decision node of a tree
def BuildTree(tree, node, k):
    ...
    for d in range(k):
        ...
        nM=tree[node]['State']['nM']
        nH=tree[node]['State']['nH']
        ...
        if nM<NM:
            ...
            while i<NM-nM:
                ...
                if tree[node]['Time']>t_1 and tree[node]['Time']<=t_2:
                    ...
                    i=i+1
                # If we cannot find an appropriate monthly user, check the next one
                else:
                    search=search+1
...
# Load the tree data from a CSV to a dictionary
OldTreeData = pd.read_csv('tree_data.csv', header=0, index_col=0, parse_dates=['Time'])
...
for i in range(len(OldTree)):
    ...
    if OldTree[i]['Type']=='D':
        ...
        OldTree[i].pop('State')
OldTree['DayOfWeek']={1:1,2:1,3:1,4:1,5:1,6:0,7:0}

# Start Optimization
Optimization(OldTree, 500)
